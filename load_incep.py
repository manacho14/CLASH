...
def normalize(image):
    ...
    return (image-float(minimum))/(maximum-minimum)

Galaxy = glob.glob('*.fits')
X = fits.open(Galaxy[0])
X = X[0].data
X = scipy.misc.imresize(X, (80, 80))
X=normalize(X)

def inception(images_size=80):
    inp=Input(shape=(1,images_size,images_size))
...
model = inception()
model.load_weights("weights-improvement-38-0.09.hdf5")
model.compile(loss= root_mean_squared_error,
                          optimizer=keras.optimizers.Adam(lr=0.00005))
probs = model.predict(X.reshape(1,1,80,80))

print('Probabilities => Spheroid: {}, Disk: {}, Irregular: {}, Point Source: {}, Unclassifiable: {}'.format(round(probs[0][0],3), 
       round(probs[0][1],3), round(probs[0][2],3), round(probs[0][3],3), round(probs[0][4],3)))